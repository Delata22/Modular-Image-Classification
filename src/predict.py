# File: src/predict.py (Final refactored version)
...
def load_components(model_dir_path: str) -> dict:
    """Loads all necessary components from a model directory."""
    print("--> Loading model components...")
    model_dir = Path(model_dir_path)
    print(model_dir)
    if not model_dir.is_dir():
        print(f"[bold red]❌ ERROR: Model directory '{model_dir}' not found.[/bold red]")
        sys.exit(1)

    # A) Load the original configuration
    original_config_path = model_dir / ".hydra" / "config.yaml"
    if not original_config_path.exists():
        print(f"[bold red]❌ ERROR: File '.hydra/config.yaml' not found in '{model_dir}'.[/bold red]")
        sys.exit(1)
    train_cfg = OmegaConf.load(original_config_path)

    # B) Find the checkpoint
    try:
        ckpt_path = next(model_dir.glob("checkpoints/*.ckpt"))
    except StopIteration:
        print(f"[bold red]❌ ERROR: No .ckpt file found in '{model_dir}/checkpoints/'.[/bold red]")
        sys.exit(1)
    
    # C) Load the class mapping
    info_path = model_dir / "train_info/data_info.yaml"
    if not info_path.exists():
        print(f"[bold red]❌ ERROR: File 'data_info.yaml' not found in '{model_dir}'.[/bold red]")
        sys.exit(1)
    with open(info_path, 'r') as f:
        class_map = yaml.safe_load(f)['class_map']

    # D) Load the model and transformations
...
# --- FUNCTION 2: The "Inference Engine" ---
def predict_single_image(image_path: Path, components: dict, device) -> dict:
    """Performs a prediction on a single image and returns a results dictionary."""
    ...
    top3_labels_list = [class_map.get(i, "unknown_class") for i in top3_indices_list]
...
def predict(cfg: DictConfig) -> None:
    ...
    # 1. Load all our tools at once
    components = load_components(model_dir_path=cfg.md)
    
    # 2. Determine the list of images to process (directory OR single file)
    if cfg.file:
        image_files = [Path(cfg.file)]
        if not image_files[0].is_file():
            print(f"[bold red]❌ ERROR: File '{cfg.file}' not found.[/bold red]")
            sys.exit(1)
    else:
    ...
    print(f"--> {len(image_files)} image(s) to predict. Results will be saved in '{output_dir}'.")

    # 3. Loop and call the inference engine
    ...
    print(f"\n[green]✅ Done! {len(image_files)} JSON files saved in {output_dir}[/green]")
